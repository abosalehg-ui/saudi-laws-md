...
def test_table_to_markdown_basic():
    table = _soup("<table><tr><th>أ</th><th>ب</th></tr><tr><td>١</td><td>٢</td></tr></table>").find("table")
    md = table_to_markdown(table)
    assert md.splitlines() == ["| أ | ب |", "| --- | --- |", "| ١ | ٢ |"]


def test_table_escapes_pipe_and_pads_ragged_rows():
    table = _soup("<table><tr><th>ع</th><th>ق</th></tr><tr><td>a|b</td></tr></table>").find("table")
    md = table_to_markdown(table)
    assert "a\\|b" in md
...
def test_prose_skips_given_lines_and_renders_headings():
    content = _soup("<div><p>English</p><h3>عنوان فرعي</h3><p>فقرة مهمة.</p><li>عنصر قائمة</li></div>").find("div")
...
def test_prose_does_not_duplicate_paragraphs_nested_in_tables():
    ...
    content = _soup(
        "<div><table><tr><th>#</th><th>ن</th></tr><tr><td>١</td><td><p>نص داخل الخلية</p></td></tr></table></div>"
    ).find("div")
...
def test_prose_does_not_duplicate_paragraphs_nested_in_li_or_blockquote():
    ...
    content = _soup("<div><ul><li><p>بند داخل قائمة</p></li></ul><blockquote><p>نص مقتبس</p></blockquote></div>").find(
        "div"
    )
    md = prose_to_markdown(content)
