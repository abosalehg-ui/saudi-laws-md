"""كشف نوع الوثيقة القانونية من عنوانها (وأحيانًا رابطها).

الأنواع مبنية على البادئات الفعلية الملاحظة في عناوين الموقعين:
نظام، لائحة (وتشمل «الدليل» و«القواعد»)، مرسوم ملكي، أمر ملكي، قرار،
اتفاقية، معايير، تعديل.
الكشف مبني على مطابقة نصية حتمية لا على استدلال لغوي، حتى يكون قابلًا
للتحقق ومستقرًا عبر الدفعات.
"""

from __future__ import annotations

import re
from urllib.parse import unquote, urlparse

# أداة التعريف تسبق أغلب البادئات في نصف العناوين ("النظام الأساس…"،
# "القواعد المنظمة…")، وكانت القواعد تطابق الصيغة النكرة وحدها — فذهبت
# 266 وثيقة إلى "أخرى" لمجرد وجود «ال». يُدرج الاختياري مرة واحدة هنا
# بدل تكراره في كل نمط.
_AL = r"(?:ال)?"


def _prefix(*words: str) -> re.Pattern[str]:
    """نمط يطابق أول العنوان بأيٍّ من الكلمات، معرَّفةً كانت أو نكرة."""
    return re.compile(rf"^{_AL}(?:{'|'.join(words)})\b")


# النوع → أنماط بادئة العنوان (تُطبَّق بالترتيب؛ أول تطابق يفوز)
_TITLE_RULES: list[tuple[str, re.Pattern[str]]] = [
    ("مرسوم ملكي", re.compile(rf"^{_AL}مرسوم\s+{_AL}ملكي")),
    ("أمر ملكي", re.compile(rf"^{_AL}[أا]مر\s+{_AL}ملكي")),
    # "قرار" إما في بداية العنوان أو بعد اسم الجهة المُصدِرة مباشرةً
    # ("وزارة كذا: قرار رقم ...") — النمط الغالب في عناوين قرارات qanoonsa
    ("قرار", re.compile(rf"^{_AL}قرار\b|:\s*{_AL}قرار\b")),
    # التعديل يُفحص قبل ما يليه: "تعديل اللائحة التنفيذية…" وثيقةُ تعديل
    # لا لائحة، وكانت تُصنَّف بالخطأ حسب الكلمة الثانية أو تسقط في "أخرى"
    ("تعديل", _prefix(r"تعديل(?:ات)?", "تحديث", r"اعتماد\s+تحديث")),
    ("اتفاقية", _prefix(r"اتفاقي(?:ة|ات)", "اتفاق", "مذكرة", "بروتوكول")),
    # الأدوات التنفيذية/الإجرائية: ما يصدر تطبيقًا لنظام أعلى منه
    ("لائحة", _prefix(
        "لائحة", "لوائح", "قواعد", "دليل", "[أا]دلة", "تعليمات", "إجراءات",
        r"آلي(?:ة|ات)", "ترتيبات",
    )),
    ("معايير", _prefix(
        "معايير", "جدول", "جداول", "اشتراطات", "ضوابط", "مواصفات",
        "شروط", "متطلبات", "كود", "نماذج", "نموذج", "وثيقة",
    )),
    # الأدوات التأسيسية/التنظيمية العليا
    ("نظام", _prefix(
        "نظام", "تنظيم", "قانون", r"سياس(?:ة|ات)", "[إا]طار", "حوكمة", "برنامج",
    )),
]

# مسار "تحديثات الأنظمة" في nezams: تعديل مادة مفردة لا نظام كامل
_UPDATE_URL_RE = re.compile(r"تحديثات[-\s]*الأنظمة|/nezam_update|تحديثات-الانظمة")

VALID_TYPES = {
    "نظام", "لائحة", "مرسوم ملكي", "أمر ملكي",
    "قرار", "اتفاقية", "معايير", "تعديل", "أخرى",
}


# بادئة عامة في تصنيف nezams الخام تُحذف لأنها لا تميّز شيئًا
_CATEGORY_PREFIX_RE = re.compile(r"^الأنظمة\s+السعودية\s*[–—-]\s*")
_CATEGORY_SPLIT_RE = re.compile(r"\s*[–—]\s*")

# صيغ بديلة لوحظت لنفس تصنيف الموقع المصدر عبر دفعات استيراد مختلفة
# (خطأ إملائي، همزة مختلفة، ترتيب كلمات، أو بادئة/لاحقة إضافية) — تُوحَّد
# إلى الصيغة الأكثر شيوعًا فعليًا في المستودع. كل إدخال هنا تحقّقتُ يدويًا
# من أن الطرفين يشيران لنفس المجال عبر مطابقة عناوين الوثائق الفعلية،
# لا تخمينًا من تشابه الاسم فقط.
_CATEGORY_ALIASES: dict[str, str] = {
    "أنظمة الأمن الداخلي": "أنظمة الأمن الداخلي والأحوال المدنية والأنظمة الجنائية",
    "أنظمة الإعلام والنشر والثقافة": "أنظمة الإعلام والثقافة والنشر",
    "الاعلام والثقافة والنشر": "أنظمة الإعلام والثقافة والنشر",
    "أنظمة البلدية والتخطيط والتطوير": "أنظمة البلدية والتخطيط والتطوير الحضري",
    "أنظمة التجارة الاقتصاد والاستثمار": "أنظمة التجارة والاقتصاد والاستثمار",
    "أنظمة الخدمات البلدية والتخطيط والتطوير": "أنظمة الخدمات البلدية والتخطيط والتطوير الحضري",
    "أنظمة الخدمة المدنية ، الموارد البشرية": "أنظمة الخدمة المدنية",
    "أنظمة الزراعة والمياة والثروات الحية": "أنظمة الزراعة والمياه والثروات الحية",
    "أنظمة المواصلات والإتصالات": "أنظمة المواصلات والاتصالات",
    "الرياضة والشباب": "أنظمة الشباب والرياضة",
    "العمل والرعاية الاجتماعية": "أنظمة العمل والرعاية الاجتماعية",
    "التنظيمات السعودية": "التنظيمات",
    # مجلد النوع "نظام" كان ينشأ من الرجوع لنوع الوثيقة عند غياب تصنيف
    # الموقع، فتتوزع التنظيمات بين مجلدَي "نظام" و"التنظيمات". قرار
    # المالك: تصنيف واحد هو "التنظيمات"
    "نظام": "التنظيمات",
    # مجلدات نوعية عامة (لوائح/قواعد) تُدمَج في مجلد النوع "لائحة" القائم،
    # فتقلّ الفئات المفردة (كلا الملفين doc_type=لائحة أصلًا)
    "لوائح وقواعد": "لائحة",
    "اللوائح التنظيمية": "لائحة",
}


def simplify_category(raw: str | None) -> str | None:
    """يبسّط تصنيف nezams الخام الطويل إلى مجال مقروء.

    مثال: "الأنظمة السعودية – أنظمة العمل والرعاية الاجتماعية" → "أنظمة
    العمل والرعاية الاجتماعية". يعيد None لمدخل فارغ حتى يتراجع النداء إلى
    بديل (نوع الوثيقة).
    """
    if not raw:
        return None
    cleaned = _CATEGORY_PREFIX_RE.sub("", raw).strip()
    # بعد حذف البادئة قد تبقى أجزاء مفصولة بشرطة؛ نأخذ أول جزء دلالي
    parts = [p.strip() for p in _CATEGORY_SPLIT_RE.split(cleaned) if p.strip()]
    result = parts[0] if parts else cleaned
    result = result.rstrip(".").strip()
    if not result:
        return None
    # "الأنظمة السعودية" وحدها (بلا مجال بعد الشرطة) لا تميّز شيئًا —
    # كل المستودع أنظمة سعودية — فتُعامل كغياب تصنيف
    if result == "الأنظمة السعودية":
        return None
    return _CATEGORY_ALIASES.get(result, result)


def resolve_category(raw: str | None, doc_type: str | None) -> str | None:
    """التصنيف النهائي: التصنيف المبسّط إن وُجد، وإلا نوع الوثيقة كبديل.

    يقلّل ما يتكدّس في مجلد "غير-مصنف" (خاصة صفحات qanoonsa بلا تصنيف).
    """
    simplified = simplify_category(raw)
    if simplified:
        return simplified
    if doc_type and doc_type != "أخرى":
        # نوع الوثيقة كمجلد يمر عبر نفس التوحيد (مثل نظام → التنظيمات)
        return _CATEGORY_ALIASES.get(doc_type, doc_type)
    return None


def classify_doc_type(title: str, url: str = "", is_decision: bool = False) -> str:
    """يرجع نوع الوثيقة من عنوانها (وربما رابطها).

    - صفحات تحديثات nezams تُصنَّف "تعديل" مهما كان العنوان.
    - القرارات (بنية أولا/ثانيا) تُصنَّف حسب العنوان أولًا؛ فإن لم يُطابق
      نمطًا معروفًا رُدّت "قرار".
    - ما لا يطابق أي نمط يُرد "أخرى".
    """
    decoded = unquote(urlparse(url).path) if url else ""
    if _UPDATE_URL_RE.search(decoded):
        return "تعديل"

    normalized = " ".join(title.split())
    for doc_type, pattern in _TITLE_RULES:
        if pattern.search(normalized):
            return doc_type

    if is_decision:
        return "قرار"
    return "أخرى"
